...
class SyncManager {
  ...
  Stream<SyncProgress> get syncStatus => _syncService.syncStatus;
  ...
  Future<void> initialize({
    ...
    try {
      ...
      _schemaManager = SchemaManager(_db);
      _syncService = SyncService(_db, _config);
      _backupManager = BackupManager(_db);
    ...
    try {
      ...
      if (_config.backupEnabled) {
        await Future.wait<dynamic>([
          ...
          _backupManager.backupPostgreDatabase(),
        ]);
      }
  ...
  Future<void> updateConfig(SyncConfig newConfig) async {
    ...
    _config = newConfig;
    _syncService.updateConfig(newConfig);
  ...
  /// Tüm tablo adlarını döndürür (public)
  Future<List<String>> getTables() async {
    return await _getTables();
  }
  ...
  /// Bir tablonun kayıt sayısını döndürür (public)
  Future<int> getTableRecordCount(String tableName) async {
    final result =
        await _db.rawQuery('SELECT COUNT(*) as count FROM $tableName');
  ...
  Future<bool> _checkConnectivity() async {
    try {
      // Yerel PostgreSQL bağlantısını kontrol et
      final postgre = await PostgreService.getInstance();
      await postgre.execute('SELECT 1');
      return true;
    } catch (e) {
      return false;
    }
