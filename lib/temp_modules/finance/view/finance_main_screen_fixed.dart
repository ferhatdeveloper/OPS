...
class _FinanceMainScreenState extends ConsumerState<FinanceMainScreen>
  ...
  Widget _buildCashTransactionsTab() {
    ...
    // Kasa özeti kartı
    final cashSummaryCard = Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
        border: Border.all(color: Colors.grey.shade300),
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('Kasa işlemi kaydedildi'),
    ...
    final recentTransactions = Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          financeState.cashTransactions.isEmpty
              ? const Center(
                child: Padding(
                  padding: EdgeInsets.symmetric(vertical: 20),
                  child: Text('Henüz kasa işlemi bulunmamaktadır'),
                ),
              )
              : ListView.builder(
                shrinkWrap: true,
                ...
                itemCount:
                    financeState.cashTransactions.length > 5
                        ? 5
                        : financeState.cashTransactions.length,
                itemBuilder: (context, index) {
                  final transaction =
                      financeState.cashTransactions[index]
                          as Map<String, dynamic>;
                  final isIncome = transaction['type'] == 'Tahsilat';
                  ...
                  return ListTile(
                    ...
                    subtitle: Text(
                      '${transaction['date']} - ${transaction['cashAccount']}',
                    ),
                    trailing: Text(
                      isIncome
                          ? '+${transaction['amount']}'
                    ...
                    onTap: () {
                      ...
                      showDialog(
                        ...
                        builder:
                            (context) => AlertDialog(
                              title: Text(
                                'İşlem Detayı: ${transaction['reference']}',
                              ),
                              content: SizedBox(
                                width: 400,
                                child: Column(
          ...
          Center(
            child: TextButton(
              onPressed: () {
                // Show all cash transactions in a dialog
                showDialog(
                  context: context,
                  builder:
                      (context) => AlertDialog(
                        title: const Text('Tüm Kasa İşlemleri'),
                        content: SizedBox(
                          width: 600,
                          height: 400,
              ...
              child: const Text('Tüm Kasa İşlemlerini Göster'),
            ),
          ),
        ],
      ),
    ...
    // Create the content widget with correctly ordered components
    final Widget cashTransactionsContent = Padding(
      padding: const EdgeInsets.all(16),
      child: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [cashSummaryCard, recentTransactions],
        ),
      ),
    );
